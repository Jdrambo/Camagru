...
if(isset($_SESSION['id'])){
    ...
    if (isset($_POST['submit']) && isset($_POST['pics_id']) && $_POST['submit'] === "like_pics"){
            ...
            if (isset($resp['likeid'])){
                if ($resp['status'] === '1')
                    $qb = $db->prepare('UPDATE `tablike` SET `status` = 0 WHERE `tablike`.`id` = :likeid');
                if ($resp['status'] === '0')
                    $qb = $db->prepare('UPDATE `tablike` SET `status` = 1 WHERE `tablike`.`id` = :likeid');
                $qb->bindValue(':likeid', $resp['likeid']);
                $qb->execute();
